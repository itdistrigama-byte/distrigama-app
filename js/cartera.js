// ── DISTRIGAMA · js/cartera.js ── Cartera CRM v6.1: etapas, seguimiento, segmentación A/B/C
...
// Cadencia de visita por segmento: A=mensual, B=bimensual, C=trimestral
const CADENCIA = { A: 30, B: 60, C: 90 };

window.setCFilter = (val, el) => {
  cFilter = val;
  document.querySelectorAll('#scr-cartera .chip[id^="cf-"]').forEach(c=>c.classList.remove('on'));
  el.classList.add('on');
  renderCartera();
};

window.setCTipo = (val, el) => {
  cTipo = val;
  document.querySelectorAll('#scr-cartera .chip[id^="cft-"]').forEach(c=>c.classList.remove('on'));
  el.classList.add('on');
  renderCartera();
};

function diasSinVisita(c) {
  if (!c.ultima_visita) return null;
  return Math.floor((NOW - new Date(c.ultima_visita.seconds*1000)) / 86400000);
}
...
window.renderCartera = async () => {
  ...
  try {
    ...
    if (cTipo) docs = docs.filter(d=>d.tipo===cTipo);
    ...
    el.innerHTML = docs.map(c=>{
      ...
      const dias = diasSinVisita(c);
      const atrasado = c.tipo && CADENCIA[c.tipo] && dias !== null && dias > CADENCIA[c.tipo];
      const tipoBadge = c.tipo ? `<span class="badge ba-${c.tipo}">${c.tipo}</span>` : '';
      const proxFechaStr = (() => {
      ...
      return `<div onclick="openFicha('${c.rif_normalizado}')" style="padding:14px 16px;border-bottom:1px solid var(--border);cursor:pointer;display:flex;justify-content:space-between;align-items:center;gap:8px">
        <div style="flex:1;min-width:0">
          <div style="font-size:14px;font-weight:700;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${c.nombre||'Sin nombre'}${tipoBadge}</div>
          <div style="font-size:11px;color:var(--muted);margin-top:2px">${c.rif_formato||c.rif_normalizado} · Ruta ${c.ruta||'—'}</div>
          ${atrasado ? `<div style="font-size:11px;color:var(--r);margin-top:3px;font-weight:600">⚠ ${dias} días sin visita (cadencia ${CADENCIA[c.tipo]}d)</div>` : ''}
...
window.openFicha = async (rif) => {
  ...
  document.getElementById('fc-seg-res').value='';
  document.getElementById('fc-seg-obs').value='';
  document.getElementById('fc-seg-fecha').value='';
  document.getElementById('fc-seg-hora').value='';
  try {
    ...
    if (c.tipo) document.getElementById('fc-tipo-badge').innerHTML = `<span class="badge ba-${c.tipo}">${c.tipo}</span>`;
...
window.closeFicha = () => { document.getElementById('modal-ficha').style.display='none'; _fichaData=null; _fichaRif=null; };

// Cambiar etapa del cliente desde la ficha
...
// Registrar visita de seguimiento desde la ficha
window.saveSeguimiento = async () => {
  ...
  try {
    ...
    document.getElementById('fc-seg-fecha').value='';
    document.getElementById('fc-seg-hora').value='';
    openFicha(_fichaRif); // refrescar stats e historial
    renderCartera();
  } catch(e) {
...
// Crear pedido con datos del cliente precargados
